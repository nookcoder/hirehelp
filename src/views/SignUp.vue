...
export default {
  ...
  methods:{
    ...
    checkID(event){
      ...
      this.$http.post(this.$store.state.host + '/api/company/signup/check', { user_id: this.user.id})
      .then(res => {
        if(res.data){
    ...
    onSumit(event){
      ...
      this.$http.post(this.$store.state.host + '/api/company/signup', { user: this.user })
    .then((res) => {
      if (res.data.success) {
        alert("No");
      } else {
